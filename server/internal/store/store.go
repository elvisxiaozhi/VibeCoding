...
func (s *Store) ListAssets(userID string) ([]model.Asset, error) {
	rows, err := s.db.Query(`
		SELECT id, user_id, symbol, category, cost_basis, current_price, quantity, currency, created_at, updated_at
	...
	for rows.Next() {
		var a model.Asset
		if err := rows.Scan(&a.ID, &a.UserID, &a.Symbol, &a.Category, &a.CostBasis, &a.CurrentPrice, &a.Quantity, &a.Currency, &a.CreatedAt, &a.UpdatedAt); err != nil {
...
func (s *Store) GetAsset(id, userID string) (model.Asset, error) {
	...
	err := s.db.QueryRow(`
		...
		Scan(&a.ID, &a.UserID, &a.Symbol, &a.Category, &a.CostBasis, &a.CurrentPrice, &a.Quantity, &a.Currency, &a.CreatedAt, &a.UpdatedAt)
	if err != nil {
...
func (s *Store) CreateAsset(a model.Asset) error {
	_, err := s.db.Exec(`
		INSERT INTO assets (id, user_id, symbol, category, cost_basis, current_price, quantity, currency, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		a.ID, a.UserID, a.Symbol, a.Category, a.CostBasis, a.CurrentPrice, a.Quantity, a.Currency, a.CreatedAt, a.UpdatedAt)
	if err != nil {
...
func (s *Store) UpdateAsset(a model.Asset) error {
	result, err := s.db.Exec(`
		...
		a.Symbol, a.Category, a.CostBasis, a.CurrentPrice, a.Quantity, a.Currency, a.UpdatedAt, a.ID, a.UserID)
	if err != nil {
